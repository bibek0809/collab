...
export default function EditorPage() {
  ...
  const siteId = useRef(`site_${user.userId}_${Date.now()}`).current;
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((d) => {
        ...
        if (d.collaborators) {
          setActiveUsers(
            d.collaborators.filter((c) => c.online).map((c) => ({
              userId: c.userId,
  ...
  const handlePresence = useCallback((update) => {
    setActiveUsers((prev) => {
      ...
      if (exists) {
        return prev.map((u) => (u.userId === update.userId ? { ...u, ...update } : u));
      }
      return [...prev, update];
    });
  }, []);
  ...
  const { connected, sendOperation, sendCursor } = useWebSocket(
    ...
    user.userId,
    user.userName,
    { onOperation: handleRemoteOp, onPresence: handlePresence, onAck: handleAck },
  );
  ...
  const handleRestore = useCallback(async () => {
    try {
      ...
      setContent(d.content ?? '');
      setCharCount((d.content ?? '').length);
      setToast({ message: 'Version restored — reload to see full state', type: 'success' });
    } catch {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-ink-50 px-4">
        <div className="text-center animate-fade-in">
          ...
          <h2 className="text-lg font-bold text-ink-900 mb-1">Document not found</h2>
    ...
    <div className="min-h-screen bg-ink-50 flex flex-col">
      ...
      <main className="flex-1 max-w-4xl w-full mx-auto px-4 py-8">
        ...
        <StatusBar
          ...
          onlineCount={activeUsers.filter((u) => u.userId !== user.userId).length + 1}
        />
      </main>
